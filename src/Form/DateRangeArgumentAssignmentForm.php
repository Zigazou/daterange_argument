...
namespace Drupal\daterange_argument\Form;
...
class DateRangeArgumentAssignmentForm extends ConfigFormBase {
  ...
  public function getFormId() {
    return 'daterange_argument_settings';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $config = $this->configFactory->getEditable('daterange_argument.settings');

    $field_names = 'date_field_names';
      ...
      foreach (Views::getAllViews() as $view) {
        ...
        foreach ($changed_filters as $filter_name) {
          ...
          $this->messenger()->addStatus(
            $this->t(
              'Updated contextual filter(s) on view %view_name.',
              ['%view_name' => $view_name]
            )
          );
          $view->save();
